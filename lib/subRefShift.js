...
export default function subRefShift(updatedModule) {
  const rehydratedModule = hydrate(dehydrate(updatedModule))
  const allShifts = []

  for (let i = 0; i < updatedModule.script.length; i++) {
    if (updatedModule.script[i].parts) {
      const shifts = []

      for (let n = 1; n <= updatedModule.script[i].parts.length; n++) {
        let shiftInfo = { old: n, new: null }

        for (let j = 0; j < updatedModule.script[i].parts.length; j++) {
          if (updatedModule.script[i].parts[j].index === n - 1) {
      ...
      allShifts.push({
        number: updatedModule.script[i].number,
        shifts: shifts
      })
    }
  }
